...
#if defined(NDEBUG)
# error "MDCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x30b58211c837d82716034afc25cc811c91d59d5b89f4483af8795385409b4c16");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MDCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MDCoin Signed Message:\n";
...
{
    // MDCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // MDCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // MDCoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // MDCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // MDCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // MDCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x58eedbcb2e1b9cf12566718dbb6a7344e21886b87eb771cf3a4fcd46ebd29edc");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "12/07/2003 MDL Coin Prima Moneda In MOLDOVA!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04acb9ee21d51db7dbccbf8a0b6edd4411ec31f79ce99926a2235f846ddcad8c1592a3ff0dc5e559a9bbefb646d7127d2f1df225fa4c170a57543f909a090c67f3") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1639510593;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086315876;

        if (fTestNet)
        {
            block.nTime    = 1639510378;
            block.nNonce   = 387768857;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x8fb565f6ff42860c22e26a34bd5f1e6a32bde9597bbdead311f7959010081da1"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc2, 0xb5, 0xde }; // MDCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MDCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MDCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MDCoinMiner(CWallet *pwallet)
{
    printf("MDCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("MDCoin-miner");
    ...
    try { loop {
        ...
        printf("Running MDCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MDCoinMiner, pwallet));
}
